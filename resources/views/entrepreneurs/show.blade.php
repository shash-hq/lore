<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div class="flex flex-col items-center justify-center text-center mb-16 relative">
            ...
            @if(auth()->id() === $user->id)
                <a href="{{ route('profile.edit') }}" class="absolute top-0 right-0 inline-block px-4 py-2 border border-[#E5E0D8] rounded-md text-sm font-sans font-medium text-[#6B6560] hover:text-[#D4542A] hover:border-[#D4542A] transition bg-white">
                    Edit Profile
                </a>
            @endif

            <img src="{{ $user->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=D4542A&background=FAF9F6&size=160' }}" alt="{{ $user->name }}" class="w-20 h-20 rounded-full border-2 border-[#E5E0D8] shadow-sm mb-6 object-cover">
            ...
            <div class="font-sans text-[14px] text-[#A09890] mb-8 font-medium">
                {{ $videos->count() }} video{{ $videos->count() !== 1 ? 's' : '' }} &middot; {{ number_format($totalViews) }} total view{{ $totalViews !== 1 ? 's' : '' }}
            </div>

            @if($user->social_links && count($user->social_links) > 0)
                <div class="flex gap-4">
                    @foreach($user->social_links as $platform => $url)
                        <a href="{{ $url }}" target="_blank" rel="noopener noreferrer" class="text-[#A09890] hover:text-[#D4542A] transition transform hover:scale-105">
                            @if(strtolower($platform) === 'twitter' || strtolower($platform) === 'x')
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 22.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.008 5.961H5.078z"></path></svg>
        ...
        @else
            <div class="columns-1 md:columns-2 lg:columns-3 gap-5 space-y-5">
                @foreach($videos as $video)
                    <div class="break-inside-avoid w-full bg-white rounded-[12px] overflow-hidden shadow-[0_2px_12px_rgba(26,24,20,0.07)] hover:-translate-y-[3px] transition-transform duration-[250ms] ease-in-out group cursor-pointer" onclick="window.location.href='{{ route('videos.show', $video->slug) }}'">
                        <div class="relative w-full aspect-video bg-gray-100 overflow-hidden">
                            <img src="https://img.youtube.com/vi/{{ $video->youtube_id }}/hqdefault.jpg" alt="{{ $video->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out">
                        </div>
                        <div class="p-[14px]">
