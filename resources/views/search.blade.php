    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-12">
        ...
        <div class="flex flex-col items-center justify-center py-10">
            <form action="{{ route('search') }}" method="GET" class="w-full max-w-2xl relative">
                ...
                @endif
                <button type="submit" class="absolute right-3 top-3 bottom-3 bg-[#D4542A] hover:bg-[#b8431e] text-white px-6 rounded-lg font-sans font-medium transition duration-150 ease-in-out flex items-center justify-center">
                    Search
                </button>
            </form>
            @if($query)
                <div class="mt-8 text-center text-[#6B6560] font-sans text-[15px]">
                    {{ $videos->total() }} result{{ $videos->total() !== 1 ? 's' : '' }} for <span class="text-[#1A1814] font-medium">'{{ $query }}'</span>
                </div>
            @endif
        </div>
        ...
        @if($videos->isEmpty())
            ...
            <div class="flex flex-col items-center justify-center py-16 px-4 text-center">
                <div class="w-20 h-20 rounded-full bg-[#F2EFE9] flex items-center justify-center mb-6">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-[#A09890]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p class="font-sans text-[16px] text-[#6B6560] max-w-md mx-auto">
                    We couldn't find any videos matching your search. Try searching for broader terms like 'fundraising', 'growth', or 'pivot'.
                </p>
